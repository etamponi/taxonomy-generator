...
class TestRawFilter(unittest.TestCase):
    ...
    def test_min_length(self):
        ...
        flt = RawFilter(min_length=3)
        self.assertEqual(expected_rci_1.term_frequencies, flt.apply(original_rci).term_frequencies)
        flt = RawFilter(min_length=2)
        self.assertEqual(expected_rci_2.term_frequencies, flt.apply(original_rci).term_frequencies)
    ...
    def test_lemmatize(self):
        ...
        flt = RawFilter(lemmatizer=WordNetLemmatizer())
        self.assertEqual(expected_rci.term_frequencies, flt.apply(original_rci).term_frequencies)

    def test_stopwords(self):
        original_rci = RawCategoryInfo("A", 100, {"a": 10, "an": 20, "do": 30, "test": 40})
        expected_rci = RawCategoryInfo("A", 100, {u"test": 40})
        flt = RawFilter(stopwords=stopwords.words("english"))
        self.assertEqual(expected_rci.term_frequencies, flt.apply(original_rci).term_frequencies)
    ...
    def test_unicode(self):
        ...
        flt = RawFilter()
        self.assertNotEqual(original.term_frequencies, flt.apply(original).term_frequencies)
        self.assertEqual(expected.term_frequencies, flt.apply(original).term_frequencies)
