...
class TestPreprocessor(unittest.TestCase):
    ...
    def test_min_length(self):
        ...
        tp = Preprocessor(min_length=3)
        self.assertEqual(expected_rci_1.term_frequencies, tp.process(original_rci).term_frequencies)
        tp = Preprocessor(min_length=2)
        self.assertEqual(expected_rci_2.term_frequencies, tp.process(original_rci).term_frequencies)
    ...
    def test_lemmatize(self):
        ...
        tp = Preprocessor(lemmatizer=WordNetLemmatizer())
        self.assertEqual(expected_rci.term_frequencies, tp.process(original_rci).term_frequencies)

    def test_stopwords(self):
        original_rci = RawCategoryInfo("A", 100, {"a": 10, "an": 20, "do": 30, "test": 40})
        expected_rci = RawCategoryInfo("A", 100, {u"test": 40})
        tp = Preprocessor(stopwords=stopwords.words("english"))
        self.assertEqual(expected_rci.term_frequencies, tp.process(original_rci).term_frequencies)
    ...
    def test_unicode(self):
        ...
        tp = Preprocessor()
        self.assertNotEqual(original.term_frequencies, tp.process(original).term_frequencies)
        self.assertEqual(expected.term_frequencies, tp.process(original).term_frequencies)
